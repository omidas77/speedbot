<?php
ob_start();
define('API_KEY','222202450:AAEIibpIwlglclgLoEIUgwKquc4y0XKTWzQ');
$the_admin_id = 0;
                ...
                'results' => json_encode([[
                    ...
                    [
                        ...
                        'input_message_content'=>[
                            'message_text'=>'@EagleInlineBannerBot getlist-'.$theListId
                        ]
                    ]])
            ]));
        }else{
...
file_put_contents('ola',ob_get_clean());
    ...
    if($userTEXT == "/help"){
        $helpTXT = "🌐      3به راهنمای ربات خوش آمدید  👍
➖➖➖➖➖➖➖
...
<pre>@EagleInlineBannerBot d6cdbea45b238632bdd6d11dcf7fe98f </pre>
...
<code>@EagleInlineBannerBot getlist-XXXXX </code>
...
👈  حالا برای من کد inline تان را بفرستید : ( فعلا فقط لیست ها قبول هستند )".$cancel_Text,
                ...
                case '2':{
                    ...
                    if(str_replace('@EagleInlineBannerBot getlist-','',$code) != $code){
                        $code_list = trim(str_replace('@EagleInlineBannerBot getlist-','',$code));
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                            ...
                            $theCommand = '@EagleInlineBannerBot getlist-'.ListTab::getCurrentMakeID($update->message->from->id);
                            makeHTTPRequest('sendMessage',[
                                'chat_id'=>$update->message->from->id,
                                'text'=>$theCommand,
                                'reply_markup'=>json_encode([
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                    ...
                    $theCommand = '@EagleInlineBannerBot getbanner-'.getCurrentMakeID($update->message->from->id);
                    makeHTTPRequest('sendMessage',[
                        'chat_id'=>$update->message->from->id,
                        'text'=>$theCommand,
                        'reply_markup'=>json_encode([
...
//                    ]);
//                    makeHTTPRequest('sendMessage',[
//                        'text'=>'@EagleInlineBannerBot getbanner-'.getCurrentMakeID($userID),
//                        'chat_id'=>$userID
//                    ]);
//                }break;
